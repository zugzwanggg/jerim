  ...
  try {
    ...
    const dbQuery = await db.query(
      `WITH combined_activities AS (
        ...
        UNION ALL
        SELECT id, user_id, description, created_at, 'Picked litter' AS activity_type FROM picked_litters WHERE user_id = $1
        UNION ALL
        SELECT id, user_id, comment, created_at, 'Tree plant' AS activity_type FROM plants WHERE user_id = $1
      )
